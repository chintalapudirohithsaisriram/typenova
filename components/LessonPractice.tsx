...
export default function LessonPractice({ lesson, showKeyboard, showFingerGuide, onComplete, onRetry }: Props) {
  ...
  const session = useTypingSession({ target: lesson.exercise, durationMs: durationSeconds * 1000, completeOnTarget: true, onComplete });
  ...
  const remainingMs = Math.max(0, durationSeconds * 1000 - session.elapsedMs);
  ...
  return <div className="lesson-practice-card">
    <div className="lesson-metrics"><div><strong>{wpm}</strong><span>WPM</span></div><div><strong>{accuracy}%</strong><span>Accuracy</span></div><div><strong>{session.mistakes}</strong><span>Mistakes</span></div><div><strong>{Math.round(session.progress)}%</strong><span>Progress</span></div><div><strong>{formatDuration(Math.ceil(remainingMs / 1000))}</strong><span>Remaining</span></div></div>
    <div className="lesson-goal-line"><span>Lesson time: {formatDuration(durationSeconds)}</span><span>Mastery: {lesson.goalAccuracy}%+ accuracy · {lesson.goalWpm}+ WPM</span></div>
    <div className="progress-track"><span style={{ width: `${session.progress}%` }} /></div>
    {!session.done ? <>
      <div className="lesson-start-note"><strong>Ready?</strong><span>Type the highlighted exercise. The timer starts on your first key and gives you {formatDuration(durationSeconds)} for this lesson.</span><button className="secondary" onClick={focus}>Focus typing area</button></div>
      ...
      <div className="lesson-live-guide"><span>Next key <strong>{session.currentChar === ' ' ? 'Space' : session.currentChar || '—'}</strong></span><span>Finger <strong>{finger}</strong></span><span>{session.running ? `${formatDuration(Math.ceil(remainingMs / 1000))} left` : 'Ready'}</span></div>
