...
function buildText(mode: ContentMode, customChars: string) {
...
export default function TypingTest({ onComplete, initialMode = 'passage', initialCustomText = '', compact = false }: Props) {
  ...
  const [customChars, setCustomChars] = useState(initialCustomText.replace(/[^a-z0-9]/gi, ''));
  const [appliedChars, setAppliedChars] = useState(initialCustomText.replace(/[^a-z0-9]/gi, ''));
  const [showKeyboard, setShowKeyboard] = useState(true);
  const [showFingerGuide, setShowFingerGuide] = useState(true);
  const [bestWpm, setBestWpm] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  const target = useMemo(() => buildText(mode, appliedChars), [mode, appliedChars]);
  ...
  const applyCustom = () => { const cleaned = customChars.replace(/[^a-z0-9]/gi, '').toLowerCase(); if (!cleaned) return; setAppliedChars(cleaned); setMode('custom'); focusInput(); };
