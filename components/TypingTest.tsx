...
function durationLabel(value: number) { return value < 60 ? `${value}s` : formatDuration(value); }
...
export default function TypingTest({ onComplete, initialDuration = 60, initialMode = 'passage', initialCustomText = '', compact = false }: Props) {
  ...
  const remainingSeconds = Math.max(0, Math.ceil((duration * 1000 - session.elapsedMs) / 1000));
