...
test('empty typing state does not claim accuracy or consistency', () => { const stats = calculateStats(0, 0, 0); assert.equal(stats.accuracy, 0); assert.equal(stats.consistency, 0); assert.equal(stats.errorRate, 0); });
test('accuracy, error rate and errors are bounded and deterministic', () => { const stats = calculateStats(90, 10, 60_000); assert.equal(stats.typed, 100); assert.equal(stats.accuracy, 90); assert.equal(stats.errorRate, 10); assert.equal(stats.errors, 10); assert.equal(stats.grossWpm, 20); assert.equal(stats.netWpm, 18); });
test('corrected key errors affect net WPM without corrupting final accuracy', () => { const stats = calculateStats(100, 0, 60_000, [], 8); assert.equal(stats.accuracy, 100); assert.equal(stats.errorRate, 8); assert.equal(stats.grossWpm, 20); assert.equal(stats.errors, 8); assert.equal(stats.netWpm, 18.4); });
