...
test('zero elapsed time is safe', () => {
  ...
  assert.equal(calculateStats(0, 0, 0).netWpm, 0);
});

test('accuracy and errors are bounded and deterministic', () => {
  const stats = calculateStats(90, 10, 60_000);
  assert.equal(stats.typed, 100);
  assert.equal(stats.accuracy, 90);
  assert.equal(stats.errors, 10);
